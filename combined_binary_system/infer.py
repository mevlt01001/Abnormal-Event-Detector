"""Inference and Timeline Visualization for Combined Binary Anomaly Detection."""
...
from combined_binary_system.dataset import load_feature_tensor
from combined_binary_system.model import DeepBinaryAnomalyHead


class CombinedBinaryAnalyzer:
    """Inference and timeline visualization engine for combined binary anomaly detection.

    Args:
        model: Trained DeepBinaryAnomalyHead.
    ...
    def render_timeline_plot(
        ...
        Styling:
        - Deep AMOLED dark theme (#0a0e17 / #05070c).
        - Distinct vivid curves: Electric Mint/Cyan (#00f5d4) or Neon Coral/Red (#ff3366).
        - No white overlay curve.
        """
        ...
        # Dynamic vibrant curve color: Electric Mint for normal, Neon Coral for detected anomaly
        curve_color = "#ff3366" if is_anom else "#00f5d4"

        # 1. Shaded detected anomaly intervals (disabled as requested)
        if show_intervals:
        ...
        # 2. Main Binary Anomaly Curve (Vivid color, no white overlay!)
        ...
        ax.set_title(f"Birleşik (UCF + XD) İkili Anomali Analizi: {vname}", color="#ffffff", fontsize=12, fontweight="bold", pad=12)
