"""Unified video processing engine powered by Decord and OpenCV.

Single source of truth for reading, temporal segmenting, sliding clip generation,
and clipping/saving anomaly segment video files without code duplication.
"""
...
class VideoProcessor:
    ...
    def __init__(
        ...
        if overlap is not None:
            ...
            calc_stride = (
                max(1, int(round(self.clip_size * (1.0 - self.overlap_ratio))))
                if self.overlap_ratio > 0.0
                else int(self.clip_size)
            )
    ...
    def extract_uniform_segments(
        ...
        # Handle short videos by interpolating authentic video frames
...
def save_segment_clips(
    ...
    """Cuts and saves video clips for detected anomaly segments.

    Args:
        video_path: Path to original source video.
        segments: List of segment dictionaries containing 'start_time' and 'end_time'.
        save_dir: Target directory where .mp4 clips will be written.
        prefix: Filename prefix for saved clips.

    Returns:
        List of absolute file paths to saved .mp4 files (or None if a clip failed).
    """
    if not segments:
        return []
    ...
    for idx, segment in enumerate(segments, start=1):
        s_time = float(segment.get("start_time", 0.0))
        e_time = float(segment.get("end_time", s_time))
        c_label = str(segment.get("top_class", "")).replace(" ", "_")
        tag = f"_{c_label}" if c_label else ""

        start_frame = max(0, int(s_time * fps))
        end_frame = min(max(0, total_frames - 1), int(e_time * fps))

        clip_name = f"{prefix}_{idx:02d}{tag}_{s_time:.1f}s-{e_time:.1f}s.mp4"
...
def get_video_duration(video_path: str) -> float:
    """Returns total duration of a video in seconds."""
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
